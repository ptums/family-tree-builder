...
export class AIService {
    ...
    async suggestFamilyNodeCreation(description, existingNodes) {
    ...
    Only include fields that can be reasonably inferred from the description. Use null for unknown values.`;
        const response = await this.openai.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: "You are a helpful assistant for managing family tree data. Return only valid JSON.",
                },
                {
                    role: "user",
                    content: prompt,
    ...
    async suggestRelationships(nodeName, existingNodes) {
        const existingNames = existingNodes.map((node) => node.name).join(", ");
    ...
    Base suggestions on common family patterns, ages, and any contextual clues.`;
        const response = await this.openai.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: "You are a helpful assistant for suggesting family relationships. Return only valid JSON.",
                },
                {
                    role: "user",
                    content: prompt,
    ...
    Return JSON: {"isValid": boolean, "suggestions": ["array of improvement suggestions"]}`;
        const response = await this.openai.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: "You are a data validation expert for family tree information. Return only valid JSON.",
                },
                {
                    role: "user",
                    content: prompt,
