...
export class FamilyTreeDatabase {
    async getAllNodes() {
        ...
        for (const { node_id, spouse_id } of spouses) {
            if (!spouseMap.has(node_id))
                spouseMap.set(node_id, []);
            spouseMap.get(node_id).push(spouse_id);
            if (!spouseMap.has(spouse_id))
                spouseMap.set(spouse_id, []);
            spouseMap.get(spouse_id).push(node_id);
        }
        const childMap = new Map();
        for (const { parent_id, child_id } of children) {
            if (!childMap.has(parent_id))
                childMap.set(parent_id, []);
        ...
        return nodes.map((node) => {
            ...
            const spouses = sm?.map((id) => ({
                id,
                type: "married",
            })) || [];
            const children = cm?.map((id) => ({
                id,
                type: "blood",
            }));
            return {
    ...
    async createNode(data) {
      ...
      INSERT INTO family_node (
        ...
        ${newId}, ${data.name}, ${data.birth || null}, ${data.birthLocation || null}, 
        ${data.death || null}, ${data.deathLocation || null}, ${normalizedFatherId}, 
        ${normalizedMotherId}, ${data.occupation || null}, ${data.profileImg || null}, ${data.gender || null}
      )
    `;
    ...
    async getNodeById(id) {
        ...
        if (nodes.length === 0)
            return null;
        const node = nodes[0];
        const spouses = await sql `SELECT spouse_id FROM spouse WHERE node_id = ${id}`;
        const children = await sql `SELECT child_id FROM child WHERE parent_id = ${id}`;
        return {
            ...node,
