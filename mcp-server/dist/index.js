    ...
    return {
        tools: [
            ...
            },
            {
                ...
                description: "Search for family members by name, location, or occupation",
                inputSchema: {
                    type: "object",
                    properties: {
                        query: {
            ...
            },
            {
                ...
                description: "Use AI to suggest potential relationships for a family member",
                inputSchema: {
                    type: "object",
                    properties: {
                        nodeName: {
            ...
            },
            {
                ...
                description: "Use AI to validate family node data and suggest improvements",
                inputSchema: {
                    type: "object",
                    properties: {
                        node: {
    ...
    try {
        switch (name) {
            ...
            case "create_family_node": {
                const result = await db.createNode(args);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "update_family_node": {
                const result = await db.updateNode(args);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "get_family_node_by_id": {
                const node = await db.getNodeById(args?.id);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "search_family_nodes": {
                const nodes = await db.searchNodes(args?.query);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "ai_suggest_family_node": {
                ...
                const suggestion = await ai.suggestFamilyNodeCreation(args?.description, existingNodes);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "ai_suggest_relationships": {
                const existingNodes = await db.getAllNodes();
                const suggestions = await ai.suggestRelationships(args?.nodeName, existingNodes);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "ai_validate_family_node": {
                const validation = await ai.validateFamilyNode(args?.node);
                return {
                    content: [
                        {
                            type: "text",
